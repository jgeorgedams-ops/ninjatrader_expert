...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private FDAX_ReversalScorer[] cacheFDAX_ReversalScorer;
		public FDAX_ReversalScorer FDAX_ReversalScorer(double strongThreshold, double goodThreshold, double weakThreshold, int swingLookback, bool showText, int circleSize, bool showDebugPanel)
		{
			return FDAX_ReversalScorer(Input, strongThreshold, goodThreshold, weakThreshold, swingLookback, showText, circleSize, showDebugPanel);
		}

		public FDAX_ReversalScorer FDAX_ReversalScorer(ISeries<double> input, double strongThreshold, double goodThreshold, double weakThreshold, int swingLookback, bool showText, int circleSize, bool showDebugPanel)
		{
			if (cacheFDAX_ReversalScorer != null)
				for (int idx = 0; idx < cacheFDAX_ReversalScorer.Length; idx++)
					if (cacheFDAX_ReversalScorer[idx] != null && cacheFDAX_ReversalScorer[idx].StrongThreshold == strongThreshold && cacheFDAX_ReversalScorer[idx].GoodThreshold == goodThreshold && cacheFDAX_ReversalScorer[idx].WeakThreshold == weakThreshold && cacheFDAX_ReversalScorer[idx].SwingLookback == swingLookback && cacheFDAX_ReversalScorer[idx].ShowText == showText && cacheFDAX_ReversalScorer[idx].CircleSize == circleSize && cacheFDAX_ReversalScorer[idx].ShowDebugPanel == showDebugPanel && cacheFDAX_ReversalScorer[idx].EqualsInput(input))
						return cacheFDAX_ReversalScorer[idx];
			return CacheIndicator<FDAX_ReversalScorer>(new FDAX_ReversalScorer(){ StrongThreshold = strongThreshold, GoodThreshold = goodThreshold, WeakThreshold = weakThreshold, SwingLookback = swingLookback, ShowText = showText, CircleSize = circleSize, ShowDebugPanel = showDebugPanel }, input, ref cacheFDAX_ReversalScorer);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.FDAX_ReversalScorer FDAX_ReversalScorer(double strongThreshold, double goodThreshold, double weakThreshold, int swingLookback, bool showText, int circleSize, bool showDebugPanel)
		{
			return indicator.FDAX_ReversalScorer(Input, strongThreshold, goodThreshold, weakThreshold, swingLookback, showText, circleSize, showDebugPanel);
		}

		public Indicators.FDAX_ReversalScorer FDAX_ReversalScorer(ISeries<double> input , double strongThreshold, double goodThreshold, double weakThreshold, int swingLookback, bool showText, int circleSize, bool showDebugPanel)
		{
			return indicator.FDAX_ReversalScorer(input, strongThreshold, goodThreshold, weakThreshold, swingLookback, showText, circleSize, showDebugPanel);
		}
	}
