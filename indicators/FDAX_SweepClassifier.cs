...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private FDAX_SweepClassifier[] cacheFDAX_SweepClassifier;
		public FDAX_SweepClassifier FDAX_SweepClassifier(double strongThreshold, double goodThreshold, double weakThreshold, double minSpike, int sweepLookback, int circleSize, bool showDebugPanel)
		{
			return FDAX_SweepClassifier(Input, strongThreshold, goodThreshold, weakThreshold, minSpike, sweepLookback, circleSize, showDebugPanel);
		}

		public FDAX_SweepClassifier FDAX_SweepClassifier(ISeries<double> input, double strongThreshold, double goodThreshold, double weakThreshold, double minSpike, int sweepLookback, int circleSize, bool showDebugPanel)
		{
			if (cacheFDAX_SweepClassifier != null)
				for (int idx = 0; idx < cacheFDAX_SweepClassifier.Length; idx++)
					if (cacheFDAX_SweepClassifier[idx] != null && cacheFDAX_SweepClassifier[idx].StrongThreshold == strongThreshold && cacheFDAX_SweepClassifier[idx].GoodThreshold == goodThreshold && cacheFDAX_SweepClassifier[idx].WeakThreshold == weakThreshold && cacheFDAX_SweepClassifier[idx].MinSpike == minSpike && cacheFDAX_SweepClassifier[idx].SweepLookback == sweepLookback && cacheFDAX_SweepClassifier[idx].CircleSize == circleSize && cacheFDAX_SweepClassifier[idx].ShowDebugPanel == showDebugPanel && cacheFDAX_SweepClassifier[idx].EqualsInput(input))
						return cacheFDAX_SweepClassifier[idx];
			return CacheIndicator<FDAX_SweepClassifier>(new FDAX_SweepClassifier(){ StrongThreshold = strongThreshold, GoodThreshold = goodThreshold, WeakThreshold = weakThreshold, MinSpike = minSpike, SweepLookback = sweepLookback, CircleSize = circleSize, ShowDebugPanel = showDebugPanel }, input, ref cacheFDAX_SweepClassifier);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.FDAX_SweepClassifier FDAX_SweepClassifier(double strongThreshold, double goodThreshold, double weakThreshold, double minSpike, int sweepLookback, int circleSize, bool showDebugPanel)
		{
			return indicator.FDAX_SweepClassifier(Input, strongThreshold, goodThreshold, weakThreshold, minSpike, sweepLookback, circleSize, showDebugPanel);
		}

		public Indicators.FDAX_SweepClassifier FDAX_SweepClassifier(ISeries<double> input , double strongThreshold, double goodThreshold, double weakThreshold, double minSpike, int sweepLookback, int circleSize, bool showDebugPanel)
		{
			return indicator.FDAX_SweepClassifier(input, strongThreshold, goodThreshold, weakThreshold, minSpike, sweepLookback, circleSize, showDebugPanel);
		}
	}
